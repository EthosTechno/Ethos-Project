<template>
  <div class="SignInView">
    <div class="Form">
      <div v-if="isExpiredLink" class="__title expired-link-title">Your create password link has expired</div>
      <div v-else>
        <div class="__title create-pass-title">Create a Password</div>
...
export default {
  ...
  beforeCreate() {
    // localStorage.clear();
    setupProfileSvc.checkUserEmailConfirm(this.route.params.id).then((res) => {
      this.isExpiredLink = res.data.data;
    });
  },
};
</script>
