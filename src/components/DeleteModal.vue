<template>
    ...
    <div class="custom-model">
        ...
        <div class="modal__content">
            <img src="../assets/modal-delete.svg" alt="delete"/>
              <span class="modal__title">Do you really want to <br>delete this {{projectName == 'projectList' ? 'project' : projectName == 'userTable' ? 'user' : 'stage'}}?</span>
...
import companySettingsSvc from '../services/companySettings.service';
...
const handleSubmitClick = () => {
  if(props.projectName == 'projectList') {
    const data = {
      "projectId": props.projectId
    }
    createProjectService.deleteProject(data).then((res) => {
  ...
  else if(props.projectName == 'projectStageList') {
    projectStageService.deleteProjectStage(props.projectId).then((res) => {
      useToast().success(res.data,{
              timeout: 3000});
      emit('close')
    }).catch((error) => {
  ...
  else if(props.projectName == 'userTable') {
    companySettingsSvc.DeleteUser(props.projectId).then((res) => {
      useToast().success(res.data,{
              timeout: 3000});
      emit('close')
    }).catch((error) => {
